...
export default function MangaCard({ manga, index }) {
  ...
  const coverStyle = imageUrl
    ? { filter: shouldBlur ? "blur(40px) brightness(0.3)" : "none", transition: "filter 0.3s ease" }
  ...
  return (
    ...
    >
      <div className="m-cover" style={coverStyle}>
        ...
        {/* Webtoon-style: UP badge for hot/new */}
        {manga.hot && <div className="m-hot">UP</div>}
        {manga.latest_source && <div className="m-source-badge">{manga.latest_source}</div>}

        {/* Rating at bottom-right like Webtoon */}
        ...
        {shouldBlur && (
          <div
            ...
            style={{ background: "rgba(13, 7, 20, 0.95)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "absolute", top: 0, left: 0, width: "100%", height: "100%", zIndex: 5, borderRadius: "inherit", cursor: "pointer", padding: "10px", textAlign: "center" }}
          >
            ...
            <button style={{ fontSize: "11px", color: "#fff", background: "rgba(239, 68, 68, 0.2)", border: "1px solid #ef4444", borderRadius: "4px", padding: "4px 10px", cursor: "pointer", fontFamily: "inherit" }}>
              Show cover?
            </button>
          </div>
        )}
        ...
        {isHidden && (
          <div className="m-hidden-overlay" onClick={(e) => e.stopPropagation()}>
            <div className="m-hidden-label">Hidden genre<br /><small style={{ opacity: 0.7 }}>({manga.g})</small></div>
            <button
              onClick={() => unhideGenre(manga.g)}
              style={{ fontSize: "10px", color: "rgba(255,255,255,.7)", background: "rgba(255,255,255,.1)", border: "1px solid rgba(255,255,255,.2)", borderRadius: "4px", padding: "3px 8px", cursor: "pointer", fontFamily: "inherit" }}
            >
              Show
            </button>
          </div>
        ...
        {!isHidden && (
          <div className="m-actions" onClick={(e) => e.stopPropagation()}>
            <button className="m-action-btn" onClick={() => toggleRead(manga.t)} title={isRead ? "Mark unread" : "Mark read"} aria-label={isRead ? "Mark as unread" : "Mark as read"}>
              {isRead ? "✓" : "○"}
            </button>
            <button className="m-action-btn" onClick={() => toggleBookmark({ id: manga.id, t: manga.t, cover: manga.cover, ongoing: manga.ongoing, rating: manga.rating, g: manga.g })} title={bookmarked ? "Remove from Library" : "Add to Library"} aria-label={bookmarked ? "Remove from Library" : "Add to Library"}>
              {bookmarked ? "★" : "＋"}
            </button>
          </div>
        )}
      ...
      {/* Webtoon-style: bold title, muted meta below */}
